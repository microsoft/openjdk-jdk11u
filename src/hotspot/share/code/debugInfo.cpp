...
void ObjectValue::print_on(outputStream* st) const {
  st->print("%s[%d]", is_object_merge() ? "merge_obj" : "obj", _id);
}
...
void ObjectValue::print_fields_on(outputStream* st) const {
  ...
  if (is_object_merge()) {
    ...
    if (!(merge_pointer->is_object() && merge_pointer->as_ObjectValue()->value()() == NULL) &&
        !(merge_pointer->is_constant_oop() && merge_pointer->as_ConstantOopReadValue()->value()() == NULL)) {
